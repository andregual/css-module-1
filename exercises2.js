...
/*
2. Add code to the function eitherFilter in the place marked "ADD CODE HERE"
 in order to achieve the desired console logs. 
...
The reduce function loops through the array and applies any operation that you can put into a function
 ...
 In this way, we could use reduce to do things like sum all the numbers in an array or multiply
  them all together.
const nums = [4, 1, 3];
...
/*
4. Construct a function intersection that compares input arrays and returns a new array
...
function intersection(array) {
  ...
  return reduce(
    array.slice(1),
    (a, b) => a.filter((elem) => b.includes(elem)),
    initial
  );
}
...
/*
5. Construct a function union that takes an input array of arrays, compares each array,
 and returns a new flat array that contains all elements. If there are duplicate elements,
  only add it once to the new array. Preserve the order of the elements starting
...
function union(array) {
  ...
  return reduce(
    ...
    (a, b) => {
      ...
      return concatArray.filter((e, index) => concatArray.indexOf(e) === index);
    },
    initial
  );
}
...
/*
6. Create a function groupBy that accepts an array and a callback, and returns an object.
  ...
  The value associated with each key will be an array consisting of all
  the elements that resulted in that return value when passed into the callback.
*/
...
function groupBy(array, callbackfn) {
  ...
  for (let elem of array) {
    const result = callbackfn(elem);

    if (!obj[result]) {
      obj[result] = [elem];
    } else {
      obj[result] = [...obj[result], elem];
    }
  }
...
/*
7. Construct a function objOfMatches that accepts two arrays and a callback.
 objOfMatches will build an object and return it. 
 To build the object, objOfMatches will test each element of the first
  array using the callback to see if the output matches the corresponding
   element (by index) of the second array. If there is a match, the element
    from the first array becomes a key in an object, and the element 
    from the second array becomes the corresponding value
*/

// ADD CODE HERE
function objOfMatches(arr1, arr2, callbackfn) {
  let obj = {};

  for (let elem of arr1) {
    let res = callbackfn(elem);
...
/*
8. Create a function goodKeys that accepts an object and a callback. 
...
// ADD CODE HERE
function goodKeys(obj, callbackfn) {
  let resultArray = [];

  for (let key in obj) {
    let res = callbackfn(obj[key]);
    if (res) {
